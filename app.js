...
app.use(passport.initialize()); // Passport 초기화 미들웨이 추가
...
app.use('/post', postRouter); // '/post' 경로로 들어오는 요청을 postRouter로 처리하도록 설정
app.use('/user', userRouter); // '/user' 경로로 들어오는 요청을 userRouter로 처리하도록 설정
...
app.use((err, req, res, next) => {
    res.locals.message = err.message; // 에러 메세지를 로컬 변수에 설정
